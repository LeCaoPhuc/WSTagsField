...
open class WSTagsField: UIScrollView {
    ...
    open var clearButtonIcon: String = "" {
        didSet {
            tagViews.forEach { $0.clearButtonIcon = self.clearButtonIcon }
        }
    }
    ...
    open func addTag(_ tag: WSTag) {
        ...
        //            print("add Tag retrurn =-=-=-=-=-=-=-=-=-=-=;")
        //            return
        //        }
        var regex =
            try! NSRegularExpression(pattern: "[$&+,:;.=\"\\?@#|/'<>^*()%!-]", options: NSRegularExpression.Options());
        
        if((regex.firstMatch(in: textField.text!, options: NSRegularExpression.MatchingOptions(), range: NSMakeRange(0, (textField.text?.count)!))) != nil) {
            return;
        }
        ...
        tagView.clearButtonIcon = self.clearButtonIcon;
    ...
    @objc open func onTextFieldDidChange(_ sender: AnyObject) {
        ...
        //        var length = (textField.text?.count)! - 1;
        //        var range =  textField.textRange(from: textField.endOfDocument, to: textField.endOfDocument);
        //        var regex =
        //            try! NSRegularExpression(pattern: ".*[^A-Za-z0-9].*", options: NSRegularExpression.Options());
        //
        //        if((regex.firstMatch(in: textField.text!, options: NSRegularExpression.MatchingOptions(), range: NSMakeRange(0, (textField.text?.count)!))) != nil) {
        //            textField.shouldChangeText(in: range!, replacementText: "");
        //        }
...
extension WSTagsField: UITextFieldDelegate {
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if((regex.firstMatch(in: string, options: NSRegularExpression.MatchingOptions(), range: NSMakeRange(0, string.count))) != nil){
            print("tetetetetetetetetteteteet");
            return false
        }
